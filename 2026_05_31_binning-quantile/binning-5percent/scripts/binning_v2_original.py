...
my_data=pd.read_csv(r'score_with_money.csv')
...
y_distribution = my_data['dpd_7'].value_counts(normalize=True)
print("y 占比分布:")
...
# 😕✅2.删除不需要分析的字段
my_data.drop(labels=['id_x','id_y','client_id','apply_id','pkid','pid',
             'serial_number','is_old','create_time_x','create_time_y',
             'risk_over_days','fact_money','fact_repay_money','expire'],
             axis=1, inplace=True, errors='ignore')

# 在 drop 之后、object 列清洗之前加这两行
money_col = my_data['money'].copy()   # 单独保存金额列
my_data.drop(columns=['money'], inplace=True)  # 从分析数据里移除，不参与分箱

#😕✅3.清洗数据表 my_data 中所有类型为字符串（object）的列，并尝试将它们转换为数值型（float64），如果失败则删除该列。
for i in my_data.columns[my_data.dtypes == 'object']:
    my_data[i] = my_data[i].astype(str).str.strip()
    my_data.loc[my_data[i].isin(['-999', '-9999', '-999999']), i] = np.nan
    try:
        my_data[i] = my_data[i].astype('float64')
    except:
        my_data.drop(columns=[i], inplace=True)
...
my_data['agr_label'] = 1
# 参数值设置
sample_range = '20260527以前'
seq = 1
# sample_type取值只有Total，表示测算全量样本，不需要配置参数sample_type_col；若还有其他样本类型，也支持同时测算
sample_type = ['Total']   #使用全量样本测算，不按逾期分组
sample_type_col = {}
sample_type_target = {'Total': ['dpd_7']}    #对全量样本的分析目标是 dpd_7（是否逾期）
target_ripe = {'dpd_7': ['agr_label']}      #指定一个字段 agr_label 表示“样本是否成熟可分析”，这里直接设为 1，代表全都是成熟样本
target_del_col = {'dpd_7': ['agr_label']}    #表示最终分析时会删除 agr_label，不作为分析变量
sub_div_bin = 0.1    #在分箱过程中，对头尾 10% 的数据做更细的分箱，便于识别极端风险人群
target_min_rate = {'dpd_7': [0.01]}   #每箱中 dpd_7=1 的样本至少要占 1%，否则舍弃
min_num = 20       #每个分箱最少要有 20 个样本，否则不成立
hit_num = 10       #即使满足 lift 和占比要求，但如果命中样本数 <10，也不建议作为规则输出
sample_type_lift = {'Total': {'dpd_7': 2.5}} #筛选出来的规则（箱）必须对逾期客户的 Lift ≥ 2.5，即风险提升效果要显著
# 举个例子理解规则筛选：
# 假设你在分析变量 年龄，系统可能会发现：
# 年龄 < 22 的客户中，逾期率为 15%
# 全体逾期率为 5%
...
'''
3.变量分箱结果明细输出函数：变量分箱明细结果”的标准化，更加美观
'''
...
'''
4.分析结果汇总输出函数:包含交替行颜色、自动调整列宽、冻结窗格、自动筛选等功能
'''
...
'''
第6步：最终筛选变量结果输出:这个函数主要用于“策略测算过程及变量筛选情况”的汇总展示。
'''
...
'''
1. 变量描述性统计分析函数；对每个变量进行质量评估和风险分布分析，判断变量是否可用于建模或规则筛选。
'''
...
'''
3. 变量效果分析和筛选：
并通过分箱分析筛选出最优的分箱阈值（Threshold）以及 Lift 值（衡量风险的倍数），以便找到最具预测能力的特征。
该函数 select_best_lift 用来根据变量的 Lift 值 和 阈值 来判断某个变量是否具有良好的预测能力，
并返回最佳的 Odds 分箱和 Threshold。
'''
...
def calculate_ks(good, bad):
    ...
    return ks
'''
4. 分析结果汇总、整合函数  ；分箱分析和规则效果分析
通过多个函数来对变量进行分箱、计算 Lift 值、IV 值以及其他统计量，最终汇总和整理分析结果
'''
# 规则效果分析和统计
...
'''
5. 变量分析步骤说明及分析结果统计
每个样本类型和坏客户定义下，所有变量的筛选情况，并计算出剩余变量占比。
filter2：一个数据框，包含变量筛选和分析的结果。
'''
...
# 变量描述性统计分析
var_select_01 = describe_stat_ana(describe_data=my_data, sample_range=sample_range,
                                 seq=seq, sample_type=sample_type, ana_people='wtt')

# 策略测算效果分析和筛选
filter2, bins_result_detail = bin_result_summary_final(
    hit_num=hit_num,
    bindata=my_data,
    var_select01=var_select_01,
    sub_div_bin=sub_div_bin,
    min_num=min_num,
...
outdata_dir = './'
excel_name = '分箱结果.xlsx'
wb = xlsxwriter.Workbook(outdata_dir + excel_name)
...
# 3. 输出变量分箱明细表
details_result_output(wb=wb, sheetname='2.变量分箱', data=bins_result_detail,
                      suoyin=0, ana_people='wtt')


# 4. 修改后的输出函数，用于输出变量效果分析和筛选表（包含IV、KS等指标）
def modified_summary_result_output(wb, sheetname, data):
    """
    输出变量效果分析和筛选表，确保包含 IV(total)、total_ks、WOE 等关键指标，避免 NaN/格式错误
    """
    ws = wb.add_worksheet(sheetname)

    headers = [
    '序号', '分析时间', '样本类型', '坏客户定义', '变量英文名', '变量中文名',
    '样本区间', '%Bad_Rate(不含缺失值)', '%Bad_Rate(包含缺失值)', '标签1',
    'Threshold', 'type', '#Obs', '%Obs', '#Bad', '%Bin_Bad_Rate',
    'Odds', 'Lift', '标签2', '标签3', 'IV(total)', 'total_ks', 'WOE(total)', '金额逾期率'
]

    for col_num, header in enumerate(headers):
        ws.write(0, col_num, header)

    for row_num, row_data in enumerate(data.to_dict('records'), 1):
        for col_num, col_name in enumerate(headers):
            value = row_data.get(col_name, '')  # 安全获取
            try:
                # 处理数值类指标，防止写入 NaN 出错
                if isinstance(value, (int, float, np.number)):
                    ws.write(row_num, col_num, float(value))
                else:
                    ws.write(row_num, col_num, value)
            except:
                ws.write(row_num, col_num, '')  # 如果报错，写空字符串

    # 设置列宽（可以根据需要调整）
    col_widths = {
        '序号': 8, '分析时间': 12, '样本类型': 12, '坏客户定义': 12,
        '变量英文名': 20, '变量中文名': 20, '样本区间': 15,
        '%Bad_Rate(不含缺失值)': 15, '%Bad_Rate(包含缺失值)': 15,
        '标签1': 8, 'Threshold': 12, 'type': 8, '#Obs': 10,
        '%Obs': 10, '#Bad': 10, '%Bin_Bad_Rate': 12,
        'Odds': 10, 'Lift': 10, '标签2': 8, '标签3': 8,
        'IV(total)': 10, 'total_ks': 10, 'WOE': 10,'金额逾期率': 12
    }

    for idx, col in enumerate(headers):
        ws.set_column(idx, idx, col_widths.get(col, 12))

# ── 计算每个变量分箱的金额逾期率 ──────────────────────────
# 把 money 和 dpd_7 拼回来
# ── 计算每个变量分箱的金额逾期率 ──────────────────────────
# 对齐索引，防止 money_col 与 my_data 行对不上
money_col = money_col.loc[my_data.index].reset_index(drop=True)
my_data_reset = my_data.reset_index(drop=True)

my_data_with_money = my_data_reset.copy()
my_data_with_money['money'] = money_col.values
my_data_with_money['dpd_7'] = my_data_reset['dpd_7']

money_result = []
for _, row in filter2.iterrows():
    var    = row['变量英文名']
    thresh = row['Threshold']
    typ    = row['type']

    if var not in my_data_with_money.columns or thresh == 'NaN':
        money_result.append(np.nan)
        continue
    try:
        if typ == '<=':
            mask = my_data_with_money[var] <= float(thresh)
        else:
            mask = my_data_with_money[var] > float(thresh)
        sub = my_data_with_money[mask]
        total_money = sub['money'].sum()
        bad_money   = sub.loc[sub['dpd_7'] == 1, 'money'].sum()
        rate = bad_money / total_money if total_money > 0 else np.nan
        money_result.append(rate)
    except:
        money_result.append(np.nan)

filter2['金额逾期率'] = money_result

# 输出变量效果分析和筛选表（包含IV、KS等指标）
modified_summary_result_output(wb=wb, sheetname='3.变量效果分析和筛选', data=filter2)
...
# 最初，所有的变量都会被包含在内。
#
# 分箱分析：通过对变量的分箱，检查每个变量是否能有效区分好客户与坏客户，或者不同类别的样本。
#
# 多阶段筛选：使用不同的标签（标签1、标签2、标签3）对变量进行逐步筛选。每个阶段的筛选是基于该变量的效果指标（如Lift、WOE、Bad Rate等）。
#
# 最终筛选结果：通过 get_summary 汇总筛选结果，得出哪些变量最终被保留下来，哪些变量被剔除。
#
# 输出结果
# 最终，代码会生成一个详细的报告，汇总哪些变量通过了筛选，哪些变量被淘汰，剩余的有用变量占总变量的比例。这些有用的变量通常具有较高的区分度和较强的预测能力，可以用于后续的模型训练或决策制定。

#是的，这些操作的核心目的就是 筛选出对目标最有用的变量，并排除掉不具有预测价值或效果较差的变量。
...
# 2. 描述性统计 + 筛选变量（你已经完成了）
#     - 缺失、分布、唯一值
#     - 变量分箱、WOE、IV、Lift、BadRate
#     - 标签1、2、3 筛选变量
#
# 3. 策略建模（下一步）
#     - Logistic 回归建评分卡（WOE编码）
#     - 或用机器学习模型（XGBoost等）
#     - 或规则树（策略路径）
#
# 4. 模型评估 & 验证
#     - AUC、KS、Lift、Recall、Precision 等指标
#     - 样本外验证（Time Split / OOT）
#
# 5. 策略输出
#     - 风控规则、评分卡、策略书
#     - 规则实施和部署（自动化引擎）
