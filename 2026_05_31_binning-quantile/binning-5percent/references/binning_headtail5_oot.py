...
输出 2 个 Sheet：Train分箱明细 + Test分箱明细（OOT_RATIO>0 时）。

使用前修改顶部参数区 (file_path, output_file, label, time_col, OOT_RATIO, drop_cols)。
"""
...
# ==================== SPLIT TRAIN/TEST ====================
if OOT_RATIO > 0 and OOT_RATIO < 1 and time_col in data.columns:
...
if actual_oot:
    ...
    print(f'  Sheet1: Train分箱明细 ({len(train_bins_detail)} 行, OOT_RATIO=0 全量)')
print("="*50)
